...
module.exports =
  ...
  errorForbidden: ->
      return Joi.object(
                  ...
                  message: Joi.string().description("The detailed error message.").required()
                ).description("Raised in case the user lacks priviledges or the API key does not posess necessary scopes/").meta(className: 'ErrorForbidden').required().options({allowUnknown: true,stripUnknown: false})
  ...
  errorNotFound: ->
      return Joi.object(
                  ...
                  message: Joi.string().description("The detailed error message.").required()
                ).description("The requested resource could not be found.").meta(className: 'ErrorNotFound').required().options({allowUnknown: true,stripUnknown: false})
